...
export const usePublicBlogs = (page = 1, limit = 10, tag?: string) => {
  return useQuery({
    queryKey: ['publicBlogs', page, limit, tag],
    queryFn: () => blogService.getPublishedBlogs(page, limit, tag),
    staleTime: 5 * 60 * 1000, 
  });
};
...
export const useAdminBlogs = (page = 1, limit = 10) => {
  return useQuery({
    ...
    queryFn: () => blogService.getAllBlogsAdmin(page, limit),
  });
};
